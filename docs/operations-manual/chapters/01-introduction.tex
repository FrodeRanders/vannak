...
includes a standalone Kafka runner that can feed \sitas{} workers, but the node
binary does not yet supervise that runner; those attach at the documented module
boundaries.
...
(\texttt{IptoRepoWriter}), the PROV-O SDL schema, and a \graft{} state machine
adapter (\texttt{ClusterStateMachine}).
